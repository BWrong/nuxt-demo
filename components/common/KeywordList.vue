  ...
  <div>
    ...
    <client-only>
      <slot v-if="pageTotal > 1" name="page" :page-now="pageNow" :page-total="pageTotal">
        <paginate :value="pageNow" :page-count="pageTotal" prev-text="上一页" next-text="下一页" :click-handler="changePage" container-class="pages" active-class="on" v-bind="pageConfig"/>
      </slot>
    </client-only>
  </div>
</template>
...
export default {
  ...
  async fetch() {
    this.loading = true;
    try {
      const res = await this.$api.keywordList({
