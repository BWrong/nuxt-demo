<template>
  <div>
    ...
    <slot v-else :list="list" :page-scope-id="pageScopeId"></slot>
    <client-only>
      <slot v-if="showPage && pageTotal > 1" name="page" :page-now="pageNow" :page-total="pageTotal">
        <paginate :value="pageNow" :page-count="pageTotal" page-class="page-item" prev-class="page-prev" next-class="page-next" prev-text="上一页" next-text="下一页" :click-handler="changePage" container-class="pages" active-class="on" v-bind="pageConfig"> </paginate>
      </slot>
    </client-only>
  </div>
</template>
